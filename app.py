...
@app.route("/update_suivi", methods=["POST"])
def update_suivi():
    dossier_id = request.form.get("id")
    suivi = request.form.get("suivi_cnaps")
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute("UPDATE dossiers SET suivi_cnaps = ? WHERE id = ?", (suivi, dossier_id))
        conn.commit()
    return "OK"
