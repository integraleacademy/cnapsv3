...
def get_stagiaire_by_id(stagiaire_id):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM stagiaires WHERE id = ?", (stagiaire_id,))
    stagiaire = cursor.fetchone()
    conn.close()
    return stagiaire

@app.route('/telecharger_attestation/<type>/<int:id>')
def telecharger_attestation(type, id):
    stagiaire = get_stagiaire_by_id(id)
    if not stagiaire:
        return "Stagiaire introuvable", 404
    if type not in ['APS', 'A3P']:
        return "Type d'attestation invalide", 400
    template_name = f"attestation_{type.lower()}.html"
    html = render_template(template_name, stagiaire=stagiaire)
    pdf = HTML(string=html).write_pdf()
    response = make_response(pdf)
    response.headers['Content-Type'] = 'application/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=attestation_{type}_{stagiaire["nom"]}.pdf'
    return response
