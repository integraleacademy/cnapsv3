...
def importer_donnees():
    if request.method == "POST":
        ...
        expected_cols = ['nom','prenom','formation','session','lien','statut','commentaire']
        ...
        with sqlite3.connect(DB_NAME) as conn:
            ...
            # Insérer ligne par ligne pour laisser SQLite gérer l'AUTOINCREMENT
            for _, row in df.iterrows():
                conn.execute("""INSERT INTO dossiers 
                    (nom, prenom, formation, session, lien, statut, commentaire)
                    VALUES (?,?,?,?,?,?,?)""", 
                    (row['nom'], row['prenom'], row['formation'], row['session'],
                     row['lien'], row['statut'] if row['statut'] else 'INCOMPLET', row['commentaire']))
