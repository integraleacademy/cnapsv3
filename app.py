...
def migrate_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        if 'date_transmission' not in columns:
            print("Ajout de la colonne 'date_transmission' dans la base...")
            cursor.execute("ALTER TABLE dossiers ADD COLUMN date_transmission TEXT;")
            conn.commit()
...
def update_date(id):
    date_transmission = request.form.get("date")
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute("UPDATE dossiers SET date_transmission = ? WHERE id = ?", (date_transmission, id))
    return redirect("/")
