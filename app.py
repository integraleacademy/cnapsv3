...
# Copier la base si elle n’existe pas déjà dans /mnt/data
if not os.path.exists("/mnt/data/cnaps.db") and os.path.exists("cnaps.db"):
    shutil.copyfile("cnaps.db", "/mnt/data/cnaps.db")
...
@app.route("/verif-bdd")
def verif_bdd():
    path = "/mnt/data/cnaps.db"
    if os.path.exists(path):
        return f"✅ La base existe bien à {path}"
    else:
        return f"❌ Aucun fichier trouvé à {path}"